...
/*   Updated: 2024/05/03 18:47:59 by bcarpent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_asset(t_data *data)
{
	...
	data->player.asset = mlx_xpm_file_to_image(data->mlxptr,
			"Assets/player.xpm", &width, &height);
	if (!data->player.asset)
		ft_error(data, "error xpm1");
	data->ground = mlx_xpm_file_to_image(data->mlxptr,
			"Assets/grass.xpm", &width, &height);
	if (!data->ground)
		ft_error(data, "error xpm2");
	data->obstacle = mlx_xpm_file_to_image(data->mlxptr,
			"Assets/tree.xpm", &width, &height);
	if (!data->obstacle)
		ft_error(data, "error xpm3");
	data->collectible = mlx_xpm_file_to_image(data->mlxptr,
			"Assets/coin.xpm", &width, &height);
	if (!data->collectible)
		ft_error(data, "error xpm4");
	data->exit = mlx_xpm_file_to_image(data->mlxptr,
		...
		while (++j < data->width)
		{
			if (data->map[i][j] == 'C')
				mlx_put_image_to_window(data->mlxptr, data->winptr,
					data->collectible, j * SIZE, i * SIZE);
			else if (data->map[i][j] == '0' || data->map[i][j] == 'E'
				|| data->map[i][j] == 'P')
				mlx_put_image_to_window(data->mlxptr, data->winptr,
					data->ground, j * SIZE, i * SIZE);
			else if (data->map[i][j] == '1')
				mlx_put_image_to_window(data->mlxptr, data->winptr,
					data->obstacle, j * SIZE, i * SIZE);
		}
	}
